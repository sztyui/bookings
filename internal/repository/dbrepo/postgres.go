...
func (m *postgresDBRepo) UpdateUser(u models.User) error {
	...
	query := `
		update users set first_name = $1, last_name = $2, email = $3, access_level = $4, updated_at = now()
	`
...
// GetReservationById returns back with one single reservation
...
func (m *postgresDBRepo) UpdateReservation(u models.Reservation) error {
	...
	query := `
		update reservations set first_name = $1, last_name = $2, email = $3, phone = $4, updated_at = now()
		where id = $5
	`
	_, err := m.DB.ExecContext(ctx, query,
		u.FirstName,
...
func (m *postgresDBRepo) InsertBlockForRoom(id int, startDate time.Time) error {
	...
	_, err := m.DB.ExecContext(ctx, query, startDate, startDate.AddDate(0, 0, 1), id, 2)
	if err != nil {
		return err
	}
	return nil
